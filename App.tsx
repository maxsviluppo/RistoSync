...
export default function App() {
  ...
  useEffect(() => {
      ...
      supabase.auth.getSession().then(async ({ data: { session } }) => {
          if (session) {
              ...
              if (isActive) { setSession(session); initSupabaseSync(); } 
      ...
      const { data: { subscription } } = supabase.auth.onAuthStateChange(async (event, session) => {
          if (session) { 
              await checkUserStatus(session.user); setSession(session); initSupabaseSync(); 
          } else { 
              setSession(null); setIsSuspended(false); setIsBanned(false); 
          }
          setLoadingSession(false);
      });
  ...
  useEffect(() => {
      const fetchGlobalSettings = async () => {
          if (!supabase) return;
          try {
              const { data: adminProfile } = await supabase.from('profiles').select('settings').eq('email', SUPER_ADMIN_EMAIL).single();
  ...
  const handleLoginWaiter = () => { if (waiterNameInput.trim()) { saveWaiterName(waiterNameInput); setRole('waiter'); } };
  const checkRoleAccess = (selectedRole: string) => { if (selectedRole === 'kitchen') setRole('kitchen'); else if (selectedRole === 'pizzeria') setRole('pizzeria'); else if (selectedRole === 'pub') setRole('pub'); else if (selectedRole === 'waiter') { const storedName = getWaiterName(); if (storedName) setRole('waiter'); else setShowLogin(true); } };
  const handleAdminAuth = () => { if (session) setShowAdmin(true); };
  
  // --- MENU & EDITOR HANDLERS ---
  const handleSaveItem = () => {
      if (!editingItem.name || !editingItem.price) return;
      const itemToSave: MenuItem = { id: editingItem.id || Date.now().toString(), name: editingItem.name, price: parseFloat(editingItem.price.toString()), category: editingItem.category || Category.ANTIPASTI, description: editingItem.description || '', ingredients: editingItem.ingredients || '', allergens: editingItem.allergens || [], image: editingItem.image, isCombo: editingItem.category === Category.MENU_COMPLETO, comboItems: editingItem.comboItems || [], specificDepartment: editingItem.specificDepartment };
      if (isEditingItem && editingItem.id) updateMenuItem(itemToSave); else addMenuItem(itemToSave);
      setMenuItems(getMenuItems()); setEditingItem({}); setIsEditingItem(false);
  };
  const confirmDelete = (item: MenuItem) => { if(confirm("Eliminare questo piatto?")) { deleteMenuItem(item.id); setMenuItems(getMenuItems()); } };
  const handleImageUpload = (e: React.ChangeEvent<HTMLInputElement>) => { const file = e.target.files?.[0]; if (file) { const reader = new FileReader(); reader.onloadend = () => { const img = new Image(); img.src = reader.result as string; img.onload = () => { const canvas = document.createElement('canvas'); const MAX_WIDTH = 1024; let width = img.width; let height = img.height; if (width > MAX_WIDTH) { height *= MAX_WIDTH / width; width = MAX_WIDTH; } canvas.width = width; canvas.height = height; const ctx = canvas.getContext('2d'); ctx?.drawImage(img, 0, 0, width, height); setEditingItem(prev => ({ ...prev, image: canvas.toDataURL('image/jpeg', 0.6) })); }; }; reader.readAsDataURL(file); } };
  const handleBulkImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => { if (!e.target.files || e.target.files.length === 0) return; const files = Array.from(e.target.files) as File[]; let updatedCount = 0; const currentMenu = getMenuItems(); let hasUpdates = false; const menuMap = new Map(); currentMenu.forEach((item, index) => { menuMap.set(item.name.toLowerCase().trim().replace(/[-_]/g, ' '), index); }); for (const file of files) { const cleanName = file.name.split('.')[0].toLowerCase().trim().replace(/[-_]/g, ' '); if (menuMap.has(cleanName)) { const index = menuMap.get(cleanName); const reader = new FileReader(); await new Promise((resolve) => { reader.onload = (evt) => { const img = new Image(); img.src = evt.target?.result as string; img.onload = () => { const canvas = document.createElement('canvas'); const MAX_WIDTH = 800; let width = img.width; let height = img.height; if (width > MAX_WIDTH) { height *= MAX_WIDTH / width; width = MAX_WIDTH; } canvas.width = width; canvas.height = height; const ctx = canvas.getContext('2d'); ctx?.drawImage(img, 0, 0, width, height); currentMenu[index].image = canvas.toDataURL('image/jpeg', 0.6); updateMenuItem(currentMenu[index]); updatedCount++; hasUpdates = true; resolve(null); } }; reader.readAsDataURL(file); }); } } if (hasUpdates) { setMenuItems(getMenuItems()); alert(`✅ Completato! Associate ${updatedCount} immagini ai piatti.`); } else { alert("⚠️ Nessuna immagine corrispondeva ai nomi dei piatti nel menu."); } };
  const handleBulkImport = (e: React.ChangeEvent<HTMLInputElement>) => { const file = e.target.files?.[0]; if (file) { const reader = new FileReader(); reader.onload = (evt) => { try { const data = JSON.parse(evt.target?.result as string); if (Array.isArray(data)) { data.forEach((item: any) => { if (item.name && item.price && item.category) { addMenuItem({ id: Date.now().toString() + Math.random(), name: item.name, price: parseFloat(item.price), category: item.category, description: item.description, ingredients: item.ingredients, allergens: item.allergens || [], image: item.image, isCombo: item.category === Category.MENU_COMPLETO, comboItems: item.comboItems || [], specificDepartment: item.specificDepartment }); } }); setMenuItems(getMenuItems()); alert(`Importati ${data.length} piatti!`); } } catch (err) { alert("Errore nel file JSON"); } }; reader.readAsText(file); } };
  const exportMenu = () => { const dataStr = "data:text/json;charset=utf-8," + encodeURIComponent(JSON.stringify(menuItems, null, 2)); const downloadAnchorNode = document.createElement('a'); downloadAnchorNode.setAttribute("href", dataStr); downloadAnchorNode.setAttribute("download", "menu_ristosync.json"); document.body.appendChild(downloadAnchorNode); downloadAnchorNode.click(); downloadAnchorNode.remove(); };
  const generateDesc = async () => { if (!editingItem.name) return; setIsGeneratingDesc(true); const desc = await generateDishDescription(editingItem.name, editingItem.ingredients || ''); if (desc) setEditingItem(prev => ({ ...prev, description: desc })); setIsGeneratingDesc(false); };
  const generateIngr = async () => { if (!editingItem.name) return; setIsGeneratingIngr(true); const ingr = await generateDishIngredients(editingItem.name); if (ingr) setEditingItem(prev => ({ ...prev, ingredients: ingr })); setIsGeneratingIngr(false); };

  // --- PROFILE, AI & SETTINGS ---
  const handleGenerateBio = async () => { if (!profileForm.name) { alert("Inserisci prima il nome del ristorante."); return; } setIsGeneratingBio(true); const bio = await generateRestaurantDescription(profileForm.name); if (bio) setProfileForm(prev => ({ ...prev, description: bio })); setIsGeneratingBio(false); };
  ...
  const handleUpdateTableCount = async (increment: number) => { const current = appSettings.restaurantProfile?.tableCount || 12; const newCount = Math.max(1, current + increment); const newSettings = { ...appSettings, restaurantProfile: { ...appSettings.restaurantProfile, tableCount: newCount } }; setAppSettingsState(newSettings); setProfileForm(prev => ({ ...prev, tableCount: newCount })); await saveAppSettings(newSettings); };
  ...
  const handleAskChef = async () => { if (!chatQuery) return; setIsChatting(true); const answer = await askChefAI(chatQuery, null); setChatResponse(answer); setIsChatting(false); };
  ...
  const analyticsData = useMemo(() => {
      ...
      dailyOrders.forEach(o => { 
          o.items.forEach(i => { 
              ...
              if (i.menuItem.ingredients) { 
                  const ingList = i.menuItem.ingredients.split(',').map(s => s.trim().toLowerCase()); 
                  ingList.forEach(ing => { if(ing) ingredientCounts[ing] = (ingredientCounts[ing] || 0) + i.quantity; }); 
              } 
          });

          if (o.createdAt && o.timestamp) {
              const diff = o.timestamp - o.createdAt;
              if (diff > 0 && diff < 3600000 * 4) {
                  totalWaitTime += diff;
                  completedOrderCount++;
              }
          }
      ...
      const avgWaitTime = completedOrderCount > 0 ? Math.floor((totalWaitTime / completedOrderCount) / 60000) : 0;
      const totalTablesServed = dailyOrders.length; 

      const topItems = Object.entries(itemCounts).sort((a, b) => b[1] - a[1]).slice(0, 5);
      const topIngredients = Object.entries(ingredientCounts).sort((a, b) => b[1] - a[1]).slice(0, 8);
      const estimatedFoodCost = revenue * 0.32; 
      const grossMargin = revenue - estimatedFoodCost;
      
      return { revenue, totalDishes, avgOrder, topItems, topIngredients, estimatedFoodCost, grossMargin, avgWaitTime, totalTablesServed };
  ...
  const handleGenerateAnalysis = async () => {
      ...
      setIsAnalyzing(true);
      const location = appSettings.restaurantProfile?.address ? appSettings.restaurantProfile.address.split(',').pop()?.trim() || "Tua Città" : "Italia";
      const topIngredientsList = analyticsData.topIngredients.map(([name]) => name);
      
      const result = await generateRestaurantAnalysis(
          { 
              totalRevenue: analyticsData.revenue, 
              totalItems: analyticsData.totalDishes, 
              avgOrder: analyticsData.avgOrder,
              avgWaitTime: analyticsData.avgWaitTime,
              totalTablesServed: analyticsData.totalTablesServed 
          }, 
  ...
  if (isBanned) return <div className="min-h-screen bg-red-950 flex flex-col items-center justify-center text-white p-8 text-center"><Shield size={64} className="mb-6 text-red-500" /><h1 className="text-4xl font-black mb-4">ACCOUNT BLOCCATO</h1><p className="text-xl mb-8">Il tuo account è stato disabilitato permanentemente.</p><button onClick={signOut} className="bg-red-700 hover:bg-red-600 px-8 py-3 rounded-xl font-bold">Esci</button></div>;
      ...
      return (
          <div className="min-h-screen bg-slate-950 text-white font-sans flex flex-col md:flex-row">
              ...
              <div className="flex-1 h-screen overflow-y-auto bg-slate-950 p-6 md:p-10 custom-scroll">
                  {/* ... (Menu, Profile, etc. sections hidden for brevity, logic unchanged) ... */}
                  {adminTab === 'menu' && ( /* ... */ <></> )}
                  {adminTab === 'notif' && ( /* ... */ <></> )}
                  ...
                  {adminTab === 'menu' && (
                      <div className="max-w-7xl mx-auto animate-fade-in relative">
                          <div className="flex flex-col md:flex-row justify-between items-center mb-8 gap-4">
                              <div><h2 className="text-4xl font-black text-white mb-2 tracking-tight">Gestione <span className="text-transparent bg-clip-text bg-gradient-to-r from-orange-400 to-red-500">Menu</span></h2><p className="text-slate-400 font-medium">Modifica il tuo menu digitale in tempo reale.</p></div>
                              <div className="flex flex-wrap gap-2">
                                  <button onClick={() => { setEditingItem({}); setIsEditingItem(!isEditingItem); window.scrollTo({ top: 0, behavior: 'smooth' }); }} className={`px-6 py-3 rounded-xl font-bold flex items-center gap-2 shadow-lg transition-transform active:scale-95 ${isEditingItem ? 'bg-slate-800 text-slate-300 border border-slate-700' : 'bg-gradient-to-r from-orange-600 to-red-600 text-white shadow-orange-900/20'}`}>{isEditingItem ? <X size={20}/> : <Plus size={20}/>} {isEditingItem ? 'Chiudi Editor' : 'NUOVO PIATTO'}</button>
                              </div>
                          </div>
                          {/* ... Rest of Menu Code ... */}
                      </div>
                  ...
                  {adminTab === 'analytics' && (
                      <div className="max-w-6xl mx-auto space-y-8 animate-fade-in pb-20">
                          {/* ... Analytics Header ... */}
                          <div className="flex flex-col md:flex-row justify-between items-center mb-6 gap-4"><div><h2 className="text-4xl font-black text-white mb-1">Business Intelligence</h2><p className="text-slate-400 text-sm font-medium flex items-center gap-1"><MapPin size={12}/> {appSettings.restaurantProfile?.address || "Località non impostata"}</p></div><div className="flex items-center gap-2 bg-slate-900 p-1 rounded-xl border border-slate-800 shadow-xl"><button onClick={() => { const d = new Date(selectedDate); d.setDate(d.getDate() - 1); setSelectedDate(d); }} className="p-3 hover:bg-slate-800 rounded-lg text-slate-400 hover:text-white transition-colors"><ChevronLeft size={20}/></button><div className="px-4 font-bold text-white flex items-center gap-2"><Calendar size={18} className="text-orange-500"/> {selectedDate.toLocaleDateString('it-IT', { weekday: 'short', day: 'numeric', month: 'long' })}</div><button onClick={() => { const d = new Date(selectedDate); d.setDate(d.getDate() + 1); setSelectedDate(d); }} className="p-3 hover:bg-slate-800 rounded-lg text-slate-400 hover:text-white transition-colors"><ChevronRight size={20}/></button></div></div>
                          
                          {/* KPI CARDS */}
                          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-4">
                              <div className="bg-slate-900 p-5 rounded-2xl border border-slate-800 shadow-lg relative overflow-hidden group hover:border-green-500/30 transition-colors"><div className="absolute top-0 right-0 p-4 opacity-5 group-hover:opacity-10 transition-opacity"><EuroIcon size={64} /></div><h3 className="text-slate-500 text-[10px] font-bold uppercase tracking-widest mb-2 flex items-center gap-2"><Banknote size={14}/> Incasso Netto</h3><p className="text-3xl font-black text-white">€ {analyticsData.revenue.toFixed(2)}</p></div>
                              <div className="bg-slate-900 p-5 rounded-2xl border border-slate-800 shadow-lg relative overflow-hidden group hover:border-blue-500/30 transition-colors"><div className="absolute top-0 right-0 p-4 opacity-5 group-hover:opacity-10 transition-opacity"><Utensils size={64} /></div><h3 className="text-slate-500 text-[10px] font-bold uppercase tracking-widest mb-2 flex items-center gap-2"><UtensilsCrossed size={14}/> Piatti Serviti</h3><p className="text-3xl font-black text-white">{analyticsData.totalDishes}</p></div>
                              <div className="bg-slate-900 p-5 rounded-2xl border border-slate-800 shadow-lg relative overflow-hidden group hover:border-pink-500/30 transition-colors"><div className="absolute top-0 right-0 p-4 opacity-5 group-hover:opacity-10 transition-opacity"><Timer size={64} /></div><h3 className="text-slate-500 text-[10px] font-bold uppercase tracking-widest mb-2 flex items-center gap-2"><Timer size={14}/> Tempo Medio Attesa</h3><p className={`text-3xl font-black ${analyticsData.avgWaitTime > 30 ? 'text-red-400' : 'text-white'}`}>{analyticsData.avgWaitTime} min</p><p className="text-[10px] text-slate-500 mt-1 font-mono">Target: &lt; 20 min</p></div>
                              <div className="bg-slate-900 p-5 rounded-2xl border border-slate-800 shadow-lg relative overflow-hidden group hover:border-cyan-500/30 transition-colors"><div className="absolute top-0 right-0 p-4 opacity-5 group-hover:opacity-10 transition-opacity"><Users size={64} /></div><h3 className="text-slate-500 text-[10px] font-bold uppercase tracking-widest mb-2 flex items-center gap-2"><Users size={14}/> Tavoli Serviti</h3><p className="text-3xl font-black text-white">{analyticsData.totalTablesServed}</p></div>
                              <div className="bg-slate-900 p-5 rounded-2xl border border-slate-800 shadow-lg relative overflow-hidden group hover:border-orange-500/30 transition-colors"><h3 className="text-slate-500 text-[10px] font-bold uppercase tracking-widest mb-2 flex items-center gap-2"><TrendingDown size={14}/> Food Cost (Est. 32%)</h3><p className="text-3xl font-black text-orange-400">€ {analyticsData.estimatedFoodCost.toFixed(0)}</p><p className="text-[10px] text-slate-500 mt-1 font-mono">Scontrino: € {analyticsData.avgOrder.toFixed(2)}</p></div>
                          </div>

                          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                              <div className="lg:col-span-2 bg-slate-900 rounded-3xl border border-slate-800 p-6 shadow-xl"><div className="flex justify-between items-center mb-6"><h3 className="font-bold text-white flex items-center gap-2"><Package className="text-blue-500"/> Consumo Materie Prime</h3><span className="text-[10px] text-slate-500 bg-slate-950 px-2 py-1 rounded border border-slate-800">Calcolato su Ingredienti Menu</span></div><div className="space-y-4">{analyticsData.topIngredients.length > 0 ? analyticsData.topIngredients.map(([name, count], i) => (<div key={name} className="relative"><div className="flex justify-between items-center text-xs font-bold mb-1 z-10 relative"><span className="capitalize text-white flex items-center gap-2"><span className={`w-5 h-5 rounded-full flex items-center justify-center text-[9px] ${i < 3 ? 'bg-blue-600 text-white' : 'bg-slate-800 text-slate-400'}`}>{i + 1}</span> {name}</span><span className="text-slate-400">{count} utilizzi</span></div><div className="h-2 bg-slate-800 rounded-full overflow-hidden"><div className="h-full bg-blue-600 rounded-full" style={{ width: `${(count / analyticsData.topIngredients[0][1]) * 100}%` }}></div></div></div>)) : (<div className="text-center py-10"><Package size={40} className="mx-auto text-slate-700 mb-2"/><p className="text-slate-500 text-sm">Nessun dato ingredienti disponibile.<br/>Assicurati di aver compilato il campo "Ingredienti" nel menu.</p></div>)}</div></div>
                              <div className="bg-slate-900 rounded-3xl border border-slate-800 p-6 shadow-xl"><h3 className="font-bold text-white flex items-center gap-2 mb-6"><Trophy className="text-yellow-500"/> Top Piatti</h3><div className="space-y-3">{analyticsData.topItems.map(([name, count], i) => (<div key={name} className="flex items-center justify-between p-3 bg-slate-950 rounded-xl border border-slate-800"><div className="flex items-center gap-3 overflow-hidden"><span className={`font-black text-sm ${i===0?'text-yellow-500':i===1?'text-slate-400':'text-orange-700'}`}>#{i+1}</span><span className="font-bold text-sm truncate">{name}</span></div><span className="font-mono text-xs font-bold bg-slate-900 px-2 py-1 rounded text-slate-300">{count}</span></div>))}{analyticsData.topItems.length === 0 && <p className="text-slate-500 text-sm italic">Nessun dato.</p>}</div></div>
                          </div>

                          <div className="bg-gradient-to-br from-indigo-900 to-purple-900 p-8 rounded-[2.5rem] border border-indigo-500/30 shadow-2xl relative overflow-hidden"><div className="absolute top-0 right-0 p-10 opacity-10"><Factory size={200} className="text-white"/></div><div className="relative z-10"><div className="flex items-center gap-4 mb-6"><div className="p-3 bg-white/10 rounded-2xl backdrop-blur-sm"><Bot size={32} className="text-white"/></div><div><h3 className="text-2xl font-black text-white">Supply Chain & Efficienza AI</h3><p className="text-indigo-200 text-sm">Analisi operativa, fornitori e food cost per <strong>{appSettings.restaurantProfile?.address ? appSettings.restaurantProfile.address.split(',').pop() : 'la tua zona'}</strong>.</p></div></div>{aiAnalysisResult ? (<div className="bg-black/30 backdrop-blur-md p-6 rounded-3xl text-sm leading-relaxed border border-white/10 text-indigo-50 whitespace-pre-line shadow-inner max-h-96 overflow-y-auto custom-scroll">{aiAnalysisResult}</div>) : (<div className="bg-black/20 p-8 rounded-3xl border border-white/5 text-center"><p className="text-indigo-200 mb-6 max-w-lg mx-auto">L'intelligenza artificiale valuterà la velocità della cucina ({analyticsData.avgWaitTime} min/tavolo), i flussi di lavoro e i consumi per suggerirti miglioramenti strutturali.</p><button onClick={handleGenerateAnalysis} disabled={isAnalyzing} className="px-8 py-4 bg-white text-indigo-900 font-black rounded-2xl shadow-xl hover:bg-indigo-50 transition-all flex items-center gap-3 mx-auto transform hover:scale-105 active:scale-95 disabled:opacity-50 disabled:transform-none">{isAnalyzing ? <Loader className="animate-spin"/> : <Sparkles/>}{isAnalyzing ? 'Analisi in corso...' : 'GENERA CONSULENZA COMPLETA'}</button></div>)}</div></div>
                      </div>
                  )}
                  {adminTab === 'share' && (
                      <div className="flex flex-col xl:flex-row gap-12 pb-20 animate-fade-in h-[calc(100vh-140px)]">
                          <div className="flex-1 space-y-6 flex flex-col justify-center max-w-xl">
                              <div><h3 className="text-4xl font-black text-white mb-3 flex items-center gap-3"><QrCode className="text-cyan-500" size={40}/> Menu Digitale</h3><p className="text-slate-400 text-lg">Il tuo menu è online. Condividilo istantaneamente con i clienti.</p></div>
                              <div className="bg-gradient-to-br from-cyan-600 to-blue-700 p-10 rounded-[2.5rem] shadow-2xl flex flex-col items-center justify-center text-center relative overflow-hidden flex-1 min-h-[400px]">
                                  <div className="absolute top-0 left-0 w-full h-full bg-[url('https://www.transparenttextures.com/patterns/cubes.png')] opacity-10"></div>
                                  <div className="bg-white p-6 rounded-3xl shadow-xl mb-8 transform hover:scale-105 transition-transform duration-500">{qrCodeUrl ? <img src={qrCodeUrl} alt="Menu QR" className="w-56 h-56 mix-blend-multiply"/> : <div className="w-56 h-56 flex items-center justify-center bg-slate-100 text-slate-400 text-xs">QR Loading...</div>}</div>
                                  <h2 className="text-3xl font-black text-white mb-2 tracking-tight">{restaurantName}</h2>
                                  <p className="text-cyan-100 font-medium mb-10">Scansiona per ordinare dal tavolo</p>
                                  <div className="flex gap-4 w-full">
                                      <button onClick={copyToClipboard} className="flex-1 bg-white/20 hover:bg-white/30 backdrop-blur-md text-white font-bold py-4 rounded-xl flex items-center justify-center gap-2 transition-all border border-white/10"><Copy size={20}/> Copia Link</button>
                                      <button onClick={handlePrintQR} className="flex-1 bg-white text-cyan-900 font-bold py-4 rounded-xl flex items-center justify-center gap-2 shadow-lg hover:bg-cyan-50 transition-all"><Printer size={20}/> Stampa</button>
                                  </div>
                              </div>
                              <div className="bg-slate-900 p-6 rounded-2xl border border-slate-800 flex items-center justify-between"><div className="flex items-center gap-4"><div className="p-4 bg-slate-800 rounded-2xl"><Globe className="text-cyan-500" size={24}/></div><div><p className="text-xs font-bold text-slate-500 uppercase">Link Pubblico</p><p className="text-white font-mono truncate w-72">{digitalMenuLink}</p></div></div><button onClick={() => window.open(digitalMenuLink, '_blank')} className="p-4 bg-slate-800 hover:bg-slate-700 text-white rounded-xl transition-colors"><ExternalLink size={20}/></button></div>
                          </div>
                          
                          {/* REALISTIC PHONE MOCKUP */}
                          <div className="flex-1 flex items-center justify-center h-full">
                              <div className="relative mx-auto border-gray-800 dark:border-gray-800 bg-gray-800 border-[14px] rounded-[2.5rem] h-[750px] w-[380px] shadow-2xl flex flex-col transform hover:scale-[1.01] transition-transform duration-500">
                                  {/* Notch */}
                                  <div className="w-[148px] h-[24px] bg-gray-800 top-0 rounded-b-[1rem] left-1/2 -translate-x-1/2 absolute z-30"></div>
                                  {/* Side Buttons */}
                                  <div className="h-[46px] w-[4px] bg-gray-800 absolute -start-[18px] top-[100px] rounded-s-lg"></div>
                                  <div className="h-[46px] w-[4px] bg-gray-800 absolute -start-[18px] top-[160px] rounded-s-lg"></div>
                                  <div className="h-[64px] w-[4px] bg-gray-800 absolute -end-[18px] top-[130px] rounded-e-lg"></div>
                                  
                                  {/* Screen Container */}
                                  <div className="rounded-[2rem] overflow-hidden w-full h-full bg-slate-950 relative ring-1 ring-white/10">
                                      <DigitalMenu restaurantId={session?.user?.id} isPreview activeMenuData={menuItems} activeRestaurantName={profileForm.name} />
                                  </div>
                              </div>
                          </div>
                      </div>
                  )}
                  {/* ... Rest of existing tabs (adminTab === 'menu', etc.) ... */}
                  {adminTab === 'menu' && (<></>)}
              </div>
          </div>
      );
...
const EuroIcon = ({ size = 24 }: { size?: number }) => (
    <svg xmlns="http://www.w3.org/2000/svg" width={size} height={size} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M4 10h12"/><path d="M4 14h9"/><path d="M19 6a7.7 7.7 0 0 0-5.2-2A7.9 7.9 0 0 0 6 12c0 4.4 3.5 8 7.8 8 2 0 3.8-.8 5.2-2"/></svg>
);
