...
export default function App() {
  ...
  const [showDeleteAllMenuModal, setShowDeleteAllMenuModal] = useState(false); // NEW: Bulk Delete Modal
  ...
  const handleConfirmDeleteAllMenu = async () => {
...
Grazie.`);
  ...
  return (
      <div className="min-h-screen bg-slate-900 flex flex-col items-center justify-center p-4 font-sans relative overflow-hidden">
        ...
        {viewOrderDetails && (
            <div className="absolute inset-0 z-[60] bg-black/80 backdrop-blur-sm flex items-center justify-center p-4 animate-fade-in">
                {/* View Order Details Modal (Unchanged) */}
        ...
        {/* NEW: DELETE ALL MENU ITEMS MODAL */}
        {showDeleteAllMenuModal && (
            <div className="absolute inset-0 z-[100] bg-red-950/80 backdrop-blur-md flex items-center justify-center p-4 animate-fade-in">
                <div className="bg-slate-900 w-full max-w-md rounded-3xl p-8 shadow-2xl border-2 border-red-600 flex flex-col items-center text-center animate-slide-up relative overflow-hidden">
                    <div className="w-16 h-16 bg-red-600/20 rounded-full flex items-center justify-center mb-4 text-red-500 border border-red-500/50"><Trash2 size={32} /></div>
                    <h2 className="text-2xl font-black text-white mb-2">SVUOTARE IL MENU?</h2>
                    <p className="text-slate-300 mb-6 text-sm">
                        Stai per eliminare <strong className="text-red-400">TUTTI I PIATTI</strong> dal menu.<br/>
                        Le impostazioni e lo storico ordini rimarranno, ma dovrai reinserire i piatti da zero.
                    </p>
                    <div className="flex flex-col gap-3 w-full">
                        <button onClick={handleConfirmDeleteAllMenu} className="w-full py-3 bg-red-600 hover:bg-red-700 text-white font-bold rounded-xl shadow-lg active:scale-95 transition-all">SÌ, ELIMINA TUTTI I PIATTI</button>
                        <button onClick={() => setShowDeleteAllMenuModal(false)} className="w-full py-3 bg-slate-800 hover:bg-slate-700 text-slate-300 font-bold rounded-xl transition-all">ANNULLA</button>
                    </div>
                </div>
            </div>
        )}
