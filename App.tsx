...
export default function App() {
  ...
  const handleSaveMenu = () => { if (editingItem.name && editingItem.price && editingItem.category) { const itemToSave = { id: editingItem.id || Date.now().toString(), name: editingItem.name, price: Number(editingItem.price), category: editingItem.category as Category, description: editingItem.description || '', allergens: editingItem.allergens || [], image: editingItem.image }; if (editingItem.id) updateMenuItem(itemToSave); else addMenuItem(itemToSave); setMenuItems(getMenuItems()); setIsEditingItem(false); setEditingItem({}); } };
                            ...
                            <div className="max-w-4xl mx-auto pb-20 animate-fade-in">
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                                    ...
                                    <div className="bg-gradient-to-br from-indigo-900 to-slate-900 p-6 rounded-3xl border border-indigo-500/30 text-center flex flex-col justify-center items-center">
                                        ...
                                        <div className="w-full bg-blue-600/50 text-white font-bold py-3 rounded-xl flex items-center justify-center gap-2 border border-blue-500/30 cursor-default">
                                            {profileForm.planType === 'Trial' ? <><Sparkles size={18}/> Prova Attiva</> : <><CreditCard size={18}/> Pagamento Attivo</>}
                                        </div>
                                        <p className="text-[10px] text-slate-500 mt-3">Per rinnovare, usa le coordinate bancarie qui sotto.</p>
                                    </div>
                                </div>
                                                        ...
                                                        <div key={idx} className="relative">
                                                            ...
                                                            <div className="h-2 bg-slate-800 rounded-full overflow-hidden">
                                                                <div 
                                                                    ...
                                                                    style={{ width: `${(dish.count / (stats.topDishes[0]?.count || 1)) * 100}%` }}
                                                                ></div>
                                                            </div>
                                                        </div>
                                                    ))}
                                                ...
                                                <div className="flex items-end justify-between h-40 gap-1 pt-4 border-b border-slate-800">
                                                    {stats.chartHours.map((h, i) => (
                                                        <div key={i} className="flex-1 flex flex-col justify-end items-center group">
                                                            {h.count > 0 && (
                                                                <div className="mb-1 text-[9px] text-white font-bold bg-slate-700 px-1 rounded opacity-0 group-hover:opacity-100 transition-opacity absolute -translate-y-6">
                                                                    {h.count}
                                                                </div>
                                                            )}
                                                            <div 
                                                                className={`w-full rounded-t-sm transition-all hover:bg-orange-400 ${h.count > 0 ? 'bg-orange-500' : 'bg-slate-800'}`} 
                                                                style={{ height: `${(h.count / stats.maxHourly) * 100}%` }}
                                                            ></div>
                                                        </div>
                                                    ))}
                                                </div>
                                                <div className="flex justify-between text-[9px] text-slate-500 mt-2 font-mono uppercase">
                                                    <span>00:00</span>
                                                    <span>12:00</span>
                                                    <span>23:00</span>
                                                </div>
                                        ...
                                        <div className="bg-gradient-to-br from-indigo-900 to-slate-900 p-6 rounded-3xl border border-indigo-500/30 shadow-lg shadow-indigo-900/20">
                                            <div className="flex items-center justify-between mb-4">
                                                <div className="flex items-center gap-3 text-indigo-400">
                                                    <Bot size={32} />
                                                    <h3 className="text-2xl font-bold text-white">AI Strategy Advisor</h3>
                                                </div>
                                                <button 
                                                    onClick={handleRunAiAnalysis} 
                                                    disabled={isAnalyzing}
                                                    className="bg-indigo-600 hover:bg-indigo-500 text-white px-6 py-3 rounded-xl font-bold shadow-lg shadow-indigo-600/30 transition-all active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2"
                                                >
                                                    {isAnalyzing ? <Loader className="animate-spin" size={18}/> : <Sparkles size={18}/>}
                                                    {isAnalyzing ? 'Analisi in corso...' : 'Genera Strategia'}
                                                </button>
                                            </div>
                                            
                                            {aiAnalysisResult ? (
                                                <div className="bg-slate-950/50 p-6 rounded-2xl border border-indigo-500/20 text-indigo-100 leading-relaxed whitespace-pre-wrap animate-fade-in shadow-inner">
                                                    {aiAnalysisResult}
                                                </div>
                                            ) : (
                                                <p className="text-slate-400 text-sm">
                                                    Clicca su "Genera Strategia" per analizzare i dati di oggi con l'Intelligenza Artificiale e ricevere consigli su come ottimizzare il servizio e aumentare il fatturato.
                                                </p>
                                            )}
                                        </div>
                                    </>
