...
const SuperAdminDashboard: React.FC<SuperAdminDashboardProps> = ({ onEnterApp }) => {
    ...
    // NUCLEAR RESET SCRIPT: DROPS AND REBUILDS FROM SCRATCH
    const getFixStructureSQL = () => `-- HARD RESET DATABASE - RISTOSYNC\n-- ATTENZIONE: Questo script CANCELLA TUTTI GLI ORDINI E I MENU per risolvere i problemi di struttura.\n-- Gli account utenti (PROFILES) vengono mantenuti.\n\n-- 1. PULIZIA TOTALE (DROP)\ndrop publication if exists supabase_realtime;\ncreate publication supabase_realtime;\n\ndrop table if exists public.orders cascade;\ndrop table if exists public.menu_items cascade;\n\n-- 2. RICREAZIONE TABELLA ORDINI\ncreate table public.orders (\n  id text primary key,\n  user_id uuid references auth.users(id) not null,\n  table_number text not null,\n  status text not null,\n  items jsonb not null,\n  timestamp bigint not null,\n  waiter_name text,\n  created_at timestamp with time zone default now() not null\n);\n\n-- 3. RICREAZIONE TABELLA MENU\ncreate table public.menu_items (\n  id text primary key,\n  user_id uuid references auth.users(id) not null,\n  name text not null,\n  price numeric not null,\n  category text not null,\n  description text,\n  ingredients text,\n  allergens jsonb,\n  image text,\n  combo_items jsonb,\n  specific_department text,\n  created_at timestamp with time zone default now() not null\n);\n\n-- 4. INDICI DI PERFORMANCE (CRITICI PER EVITARE TIMEOUT)\ncreate index idx_orders_user_id on public.orders(user_id);\ncreate index idx_orders_created_at on public.orders(created_at desc);\ncreate index idx_menu_items_user_id on public.menu_items(user_id);\n\n-- 5. RLS (SICUREZZA)\nalter table public.orders enable row level security;\nalter table public.menu_items enable row level security;\nalter table public.profiles enable row level security;\n\n-- 6. POLICY (REGOLE DI ACCESSO - RESET COMPLETO)\ndrop policy if exists "Enable all for users" on public.orders;\ndrop policy if exists "Enable all for users" on public.menu_items;\ndrop policy if exists "Public profiles are viewable by everyone" on public.profiles;\ndrop policy if exists "Users can insert their own profile" on public.profiles;\ndrop policy if exists "Users can update own profile" on public.profiles;\n\ncreate policy "Enable all for users" on public.orders for all using (auth.uid() = user_id);\ncreate policy "Enable all for users" on public.menu_items for all using (auth.uid() = user_id);\ncreate policy "Public profiles are viewable by everyone" on public.profiles for select using (true);\ncreate policy "Users can insert their own profile" on public.profiles for insert with check (auth.uid() = id);\ncreate policy "Users can update own profile" on public.profiles for update using (auth.uid() = id);\n\n-- 7. ABILITAZIONE REALTIME\nalter publication supabase_realtime add table public.orders;\nalter publication supabase_realtime add table public.menu_items;\nalter publication supabase_realtime add table public.profiles;`;
